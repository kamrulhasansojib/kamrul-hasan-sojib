      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 pt-4">
        ...
        {activeTab === "experience" && (
          <div className="max-w-5xl mx-auto space-y-8 relative">
            <div className="hidden sm:block absolute top-6 bottom-6 left-6 sm:left-8 w-[2px] bg-gradient-to-b from-blue-500/40 via-cyan-500/20 to-transparent" />

            {workExperience.map((item) => (
              <div key={item.id} className="relative pl-0 sm:pl-20 group">
                {/* Node Icon Circle (hidden on mobile) */}
                <div className="hidden sm:flex absolute left-3 sm:left-5 top-2 -translate-x-1/2 w-9 h-9 rounded-full bg-zinc-950 border border-zinc-700 group-hover:border-cyan-400 group-hover:bg-cyan-950/90 transition-all items-center justify-center text-cyan-400 z-10 shadow-xl shadow-cyan-950/50">
                  <Briefcase className="w-4 h-4" />
                </div>

                {/* Card Container */}
                <div className="p-5 sm:p-9 lg:p-10 rounded-2xl sm:rounded-3xl bg-zinc-950/80 border border-zinc-800/80 hover:border-cyan-500/50 transition-all duration-300 shadow-2xl backdrop-blur-xl relative overflow-hidden group-hover:shadow-cyan-950/30">
                  ...
                  <div className="space-y-3 pt-2">
                    ...
                    <ul className="grid grid-cols-1 md:grid-cols-2 gap-2.5">
                      {item.achievements.map((ach, idx) => (
                        <li
                          key={idx}
                          className="flex items-start gap-2.5 text-sm text-zinc-200 bg-zinc-900/40 p-2.5 sm:p-3 rounded-xl border border-zinc-800/60"
                        >
        ...
        {activeTab === "education" && (
          <div className="max-w-5xl mx-auto space-y-8 relative">
            <div className="hidden sm:block absolute top-6 bottom-6 left-6 sm:left-8 w-[2px] bg-gradient-to-b from-purple-500/40 via-purple-500/20 to-transparent" />

            {educationItems.map((item) => (
              <div key={item.id} className="relative pl-0 sm:pl-20 group">
                {/* Node Icon Circle (hidden on mobile) */}
                <div className="hidden sm:flex absolute left-3 sm:left-5 top-2 -translate-x-1/2 w-9 h-9 rounded-full bg-zinc-950 border border-zinc-700 group-hover:border-purple-400 group-hover:bg-purple-950/90 transition-all items-center justify-center text-purple-400 z-10 shadow-xl shadow-purple-950/50">
                  <GraduationCap className="w-4 h-4" />
                </div>

                <div className="p-5 sm:p-9 lg:p-10 rounded-2xl sm:rounded-3xl bg-zinc-950/80 border border-zinc-800/80 hover:border-purple-500/50 transition-all duration-300 shadow-2xl backdrop-blur-xl relative overflow-hidden group-hover:shadow-purple-950/30">
                  ...
                  <div className="flex flex-col md:flex-row md:items-start justify-between gap-4 pb-4 border-b border-zinc-900/80">
                    <div>
                      ...
                      <p className="text-purple-400 font-semibold text-base sm:text-lg mt-1">
                        {item.institution}
                      </p>
                    </div>
