  ...
  return (
    <section id="certifications" className="py-24 bg-black text-white border-b border-zinc-900/80 relative overflow-hidden">
          ...
          {filteredCerts.length > 1 && (
            <div className="mt-8 flex items-center justify-between border-t border-zinc-900 pt-4">
              ...
              <div className="text-xs font-mono text-zinc-500">
                Certification <span className="text-cyan-400 font-bold">{activeCardIndex + 1}</span> of <span className="text-zinc-300">{filteredCerts.length}</span>
              </div>
      ...
      {activePreview && (
        ...
        >
          <div 
            className="relative max-w-3xl w-full bg-zinc-950 border border-zinc-800 rounded-3xl shadow-2xl p-6 sm:p-8 max-h-[90vh] overflow-y-auto my-auto"
            onClick={e => e.stopPropagation()}
          >
            {/* Modal Header */}
            <div className="flex items-start justify-between pb-4 mb-5 border-b border-zinc-800">
              <div>
                <div className="inline-flex items-center gap-2 px-2.5 py-0.5 rounded-full text-xs font-mono text-cyan-400 bg-cyan-950/60 border border-cyan-500/20 mb-1">
            ...
            {/* Modal Footer Info */}
            <div className="mt-5 flex items-center justify-between text-xs text-zinc-400 pt-3 border-t border-zinc-900">
              <span className="font-mono bg-zinc-900 px-3 py-1.5 rounded-lg border border-zinc-800">
                Issued: <strong className="text-zinc-200">{activePreview.issueDate}</strong>
              </span>

              <button
                type="button"
                onClick={() => setActivePreview(null)}
                className="px-4 py-2 rounded-xl bg-zinc-900 hover:bg-zinc-800 text-zinc-200 border border-zinc-800 font-bold text-xs transition-colors cursor-pointer"
              >
                Close
              </button>
            </div>
